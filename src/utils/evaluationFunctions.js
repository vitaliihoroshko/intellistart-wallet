export const evaluatePasswordProgress = (value, dataCallback, visibilityCallback) => {
  ...
  else if (value.length === 9) dataCallback({ progress: 50, tooltip: 'Password is average' });
  else if (value.length === 10) dataCallback({ progress: 70, tooltip: 'Password is average' });
  else if (value.length === 11) {
    dataCallback({ progress: 90, tooltip: 'Password is strong enough' });
  } else if (value.length === 12) dataCallback({ progress: 100, tooltip: 'Password is strong' });

export const evaluatePasswordProgress = (value, dataCallback, visibilityCallback) => {
  if (value.length === 6) {
    dataCallback({ progress: 20, tooltip: 'Password is very weak' });
  } else if (value.length === 7) {
    dataCallback({ progress: 30, tooltip: 'Password is weak' });
  } else if (value.length === 8) {
    dataCallback({ progress: 40, tooltip: 'Password is weak' });
  } else if (value.length === 9) {
    dataCallback({ progress: 60, tooltip: 'Password is average' });
  } else if (value.length === 10) {
    dataCallback({ progress: 80, tooltip: 'Password is average' });
